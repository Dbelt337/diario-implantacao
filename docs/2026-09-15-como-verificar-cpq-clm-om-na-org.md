...
### Quem tem as PSLs de Product Catalog Management / Unified Catalog / Context Service em produção (15/09)
Os mesmos 5 usuários nas quatro PSLs: Alan Rodrigues Pinheiro Silva, Bruno Santos De Andrade, Caio Savoia Sarkisian e João Pedro Galvão e Albuquerque Lima (perfil Field Service Admin) e Davi Israel de Abreu (System Administrator). É o time de administração da org, não usuários de negócio; a atribuição parece vir do pacote de permissões de admin, não de uso do catálogo novo. Para fechar a dúvida, contar registros do catálogo novo (objetos padrão do Product Catalog Management): `SELECT COUNT() FROM ProductCatalog`, `SELECT COUNT() FROM ProductClassification`, `SELECT COUNT() FROM ProductSellingModel`. Zero em todos = catálogo novo não está em uso e a decisão pelo EPC do Vlocity CMT segue sem conflito.

### Depuração da licença de OM (pedido do Gerson, 15/09: "as licenças estão no novo aditivo, mas tínhamos uma cortesia com vencimento para 2028")
O que a org mostra hoje contra a hipótese da cortesia até 2028:
1. Em produção, o único direito de OM é "Maximum B2C orders submitted via Industries Order Management", **Start 11/06/2025, End 16/06/2026, Frequency Once, Allowance 0, 252 usadas, último uso 29/05/2026**. Todos os outros direitos e PSLs da org terminam em 17/06/2028 (fim do contrato). Ou seja, a cortesia de OM foi provisionada por 12 meses, não até 2028; o que vale até 2028 é o contrato principal.
2. O case #473919801 foi aberto em 23/06/2026, sete dias depois do vencimento de 16/06. A decomposição parou de funcionar exatamente quando a cortesia venceu.
3. A preprod (00DHZ000006zyyM) mostrava OrderManagement + add-ons em junho porque sandbox copia as licenças da produção na data do refresh; se o refresh foi antes de 16/06, ela ficou com a foto antiga. Conferir em Setup > Sandboxes a data do último refresh da preprod.
4. Não há PSL de Order Management em produção (lista de 15/09). A "Platform License: OrderManagement" que o suporte citou pode ser uma licença de plataforma (UserLicense/Feature License), não PSL: conferir com `SELECT Name, LicenseDefinitionKey, TotalLicenses, UsedLicenses, Status FROM UserLicense ORDER BY Name` nas duas orgs.
5. Se o novo aditivo já foi assinado, a provisão ainda não chegou à produção: a linha de ordens B2C continua com End Date 16/06/2026 e Allowance 0 e não existe linha de ordens B2B. O AE precisa confirmar a provisão (o suporte já tinha orientado isso em 26/06).
O que pedir ao Gerson/AE: (a) o texto do aditivo/ordem de compra da cortesia de OM com a data de término; (b) o número do pedido do novo aditivo e a data prevista de provisão; (c) print de Company Information da preprod para comparar.

### Catálogo novo (Product Catalog Management) em produção: sem uso (15/09)
`ProductCatalog`, `ProductClassification` e `ProductSellingModel` sem registros em produção. As PSLs de PCM/Unified Catalog/Context Service dos 5 admins são atribuição de pacote de permissões, não uso. Decisão de catálogo do projeto (EPC do Vlocity CMT) sem conflito. Ambientes conhecidos: **produção** (prod-brasiltecpar), **preprod** (00DHZ000006zyyM, staging) e **radardev** (sandbox de desenvolvimento, informada pelo Diego em 15/09). As mesmas verificações de licença/pacote valem para a radardev antes de qualquer build das works B2B.
