...
Script 27 fase 2 (15/09 ~18h, só SENAC): **item de orquestração "Aprovação - Comercial" reatribuído ao Samuel Helbig** (conferido no DEPOIS). O ApprovalWorkItem espelho não aceita update ("User cannot edit Approval Work Item Standard Fields"), fica a cargo da plataforma. O campo `ManagerAccount__c` não pôde ser gravado: regra de validação "não é possível ao usuário 'Vendedor/GR' editar uma Oportunidade/Cotação enquanto o registro está em aprovação" (disparou até para o admin); corrigir depois da aprovação. Efeito colateral a conferir: `Approval.unlock` executou e o `lock` não (update falhou no meio), a oportunidade pode ter ficado destravada; script 28 confere e retrava. PATRUS não incluída.
Script 28 (18:01 e 18:03): a oportunidade estava de fato destravada após o 27; **retravada com sucesso** (`Approval.lock`, travada agora = true). Estado final da SENAC: item de orquestração "Aprovação - Comercial" com Samuel Helbig (17:58); item espelho AWI-000011685 continua com Hermes (não editável pela API); `ManagerAccount__c` continua Hermes (bloqueado pela regra de validação até a aprovação concluir); fase "Aprovação comercial", Send4Approval = true. Pendências: retorno do Samuel (aprova pelo componente na oportunidade?), PATRUS, fórmula da regra de validação, defeito do envio duplo, log do script 21.

**21/09, erro ao aprovar a SENAC** ("Evento do orquestrador: o valor da carga util do evento e invalido: StepInstanceId"):
consequencia do script 27. O item da orquestracao (0jfV20000007worIAA) ficou com o Samuel e o espelho ApprovalWorkItem
AWI-000011685 com o Hermes; a plataforma valida o evento de aprovacao contra os dois e rejeita. Correcao no script 51: devolve
o item da orquestracao ao Hermes (alinha) e reatribui ao Samuel pela acao padrao `reassignApprovalWorkItem` (Invocable.Action),
que atualiza os dois registros; testado na staging (item 00000192, ida e volta). Licao: nunca reatribuir FlowOrchestrationWorkItem
direto; usar a acao padrao. As submissoes AS-000022205 e AS-000022844 (Errored, orquestracoes em Error) sao o defeito do envio
duplo, sem relacao com este erro.
