...
RN-04 Entrada na jornada por Salesforce Data Event no objeto "Envio para Assinatura" (criação, Status = Pendente), quem entra = Contact do lookup Signatário; dados da jornada: ID do envio, tipo, número da cotação/contrato, ID da Oportunidade, GR e e-mail do GR, link do documento. Journey Settings: Contact Entry = "Re-entry only after exiting" (o mesmo signatário pode ter envios sucessivos, nunca concorrentes na mesma jornada); Default Email Address = "email attribute from Contacts", para a correção de e-mail feita pelo GR (RN-07) valer no próximo disparo.
RN-05 Quebra da cadência: callback de assinatura (W-000105), aceite da proposta (Quote.Status = Aceita, W-000098), recusa ou expiração de 30 dias (W-000098/W-000122) atualizam Status do registro para Assinado/Recusado/Expirado. Como a Data Extension do Data Event é preenchida só na entrada, a jornada verifica o Status atual do envio (objeto sincronizado pelo Marketing Cloud Connect) num Decision Split imediatamente antes de cada e-mail e sai quando Status ≠ Pendente. Nenhum envio ocorre após a saída.
RN-06 Registro: cada disparo gera Task Completed na Oportunidade (WhatId = ID da Oportunidade do Journey Data) com assunto "Lembrete de assinatura D+N", criada pela Task/Object activity da jornada. Essa atividade é assíncrona e pode levar até 24 horas; a auditoria do envio em tempo real fica no Marketing Cloud, e o Envio para Assinatura recebe "Último lembrete enviado" por Object activity (Simple Update).
...
RN-09 Limites e pré-requisitos: Marketing Cloud Connect na versão 5.496 ou superior; no máximo 80 jornadas por objeto; o objeto e o público do entry source não podem ser trocados depois de criados; Test Mode não funciona com Salesforce Entry Source nem com "No re-entry", então o teste é em sandbox conectada com registros reais; o objeto Envio para Assinatura precisa estar na sincronização do Marketing Cloud Connect para o Decision Split da RN-05; atividades Sales Cloud só retentam erro UNABLE_TO_LOCK_ROW.
...
Riscos: callback de assinatura atrasado ou sincronização do Marketing Cloud Connect fora de hora gera lembrete indevido a quem já assinou (mitigado pelo Decision Split no momento do envio); fuso da org precisa ser America/Sao_Paulo para a régua bater com 08:00; a Task na Oportunidade pode aparecer com até 24 horas de atraso.
...
| Critério 2 — D+7 às 08:00 | New | Dado um envio com Status Pendente há exatos 7 dias, quando o relógio marca 08:00 de Brasília, então o signatário recebe o e-mail do template de proposta com cópia ao GR, o campo "Último lembrete enviado" do envio passa a D+7 e uma Task Completed "Lembrete de assinatura D+7" aparece na Oportunidade em até 24 horas. |
| Critério 8 — E-mail corrigido | New | Dado um envio pendente cujo signatário teve o e-mail corrigido pelo GR em D+3, quando o D+4 dispara, então o e-mail vai para o endereço corrigido do Contact. |
