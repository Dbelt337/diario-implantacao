...
Task colateral (scripts 32/33): o trigger criou em 16/09 18:27 a task "Aguardando contrato" (fila Backoffice, vencimento 27/03/2026) na FIREWALL GRUPO V ARAUCARIA; já existia a mesma task, aberta desde 12/03/2026 (Automated Process, dona Erica Santos Silva, mesmo vencimento). Duplicata: excluída pelo script 32 com EXCLUIR = true. Observação: a oportunidade está em "Aguardando contrato" desde março com a task original vencida há quase 6 meses.

**Encerramento (16/09 18:32)**: task duplicada excluída (Lixeira). Chamado concluído: 33 contas e 13 oportunidades com Gerente da conta = Rodrigo Nascimento Piccolo; nenhum dono alterado; nenhuma regra alterada; nenhum resíduo. Itens de melhoria observados (não executados): trigger de Opportunity recria a task de fase a cada edição, com vencimento copiado de campo antigo; oportunidade FIREWALL GRUPO V ARAUCARIA em "Aguardando contrato" desde março com task vencida.

## Chamado da Tayza 2 (16/09 manhã): "não consigo ver esta opp" (conta SAMAE)

Script 34: a conta SERVIÇO AUTÔNOMO MUNICIPAL DE ÁGUA E ESGOTO - SAMAE (001V200000lodzIIAQ) existe, dona Priscila De Lima, RT B2B PJ, sem gerente da conta, cluster ALT/GGNET, criada em 29/11/2025 por Caio Konichi e nunca alterada. A Tayza (perfil B2B - Backoffice, papel B2B Arquiteto Backoffice, cluster AVATO) tem acesso total. **A conta não tem nenhuma oportunidade, nem na Lixeira, nem cotação, nem contrato.** Não há registro "corrompido": a oportunidade está em outra conta (duplicata), foi excluída em definitivo ou nunca foi salva. Script 35 procura contas duplicadas, oportunidades com SAMAE no nome e as últimas oportunidades da Tayza.
Print da Tayza (16/09): é a **página da conta** que não carrega ("Ocorreu um problema... Server Error"), no app BTP - B2B.B2G.B2W; outras contas abrem. Conclusão: componente da record page quebrando com os dados desta conta. Script 36 (leitura) despeja todos os campos, verifica lookups quebrados e compara com uma conta irmã. Passo seguinte: reproduzir como admin e capturar o debug log da abertura da página para achar a exceção do componente.
Script 36 (16/09): SAMAE tem 52 campos preenchidos contra 82 da conta irmã (ELISEU KOPP, mesma dona e RT, abre normalmente). Faltam na SAMAE: cidade, estado, CEP, país e geolocalização dos endereços (só a rua "R.DOUTOR PENIDO, 297"), ServiceTerritory__c, ClientType__c, CNAE, LegalEntityType__c, AccountManager__c, IntegrationPerformed__c = false, IsActive = false. Nenhum lookup quebrado (RecordType/User não têm IsDeleted, checagem não se aplica). Sem contatos, casos, ativos, contas filhas, tarefas ou arquivos. Hipótese: componente da record page (endereço/território/integração) não trata endereço sem cidade/CEP. Confirmação depende do debug log da abertura da página.
**Encerrado (16/09)**: era o navegador da Tayza (cache/sessão); em outro navegador a página abriu. Nada alterado na org. Fica registrado, como observação de qualidade de dados, que a conta SAMAE veio da carga de 29/11/2025 sem cidade/estado/CEP, território, tipo de cliente, CNAE e gerente da conta, e está marcada como inativa.
