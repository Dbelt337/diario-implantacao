...
| **Espec/Estoque físico** | `VehicleDefinition` (espec do modelo/versão, `ProductId` → Product2) e `Vehicle` (unidade com VIN) | **VehicleDefinition: 223 carregadas ✅** (1:1 com o catálogo, habilita a Búsqueda de Vehículos). `Vehicle` (estoque físico por VIN) permanece para a fase de estoque, alimentado pelo SAP/DMS |
