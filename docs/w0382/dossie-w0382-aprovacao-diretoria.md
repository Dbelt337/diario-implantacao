...
| 7 | Update rodava e o registro voltava ao estado anterior (rollback silencioso) | Notificação final do `OpportunityStageApprovalProcess_B2B` falhava com "recipientIds deve estar ativo" (dono/gerente da opp inativo em staging) → fault sem tratamento desfazia a transação inteira, incluindo o update já feito | **v8**: `deploy-w0382-v8-notificacao-fault-path.zip` — fault path na ação de notificação (`SwallowNotificationFault`); notificação que falha não desfaz mais o update |
