...
2. **PSG Administrador com 14 atribuições ativas vs. 6 admins** — CONFIRMADO em 25/08: os 14 são 5 perfis admin (incl. as 2 integrações), **os 5 usuários do perfil Coordenação (todos)**, Isabela Menezes (Gerência) e 3 devs/consultores externos com Usuário Padrão (Dev Dreamm, Jean Livero/Everymind, Rodrigo Xavier/Hablla). Na "redefinição de acessos" da gestão comercial trocaram o perfil, mas o PSG Administrador permaneceu — candidato mais provável para o descongelamento acidental de usuário (confirmar autor no SetupAuditTrail). Correção sequenciada: (a) criar PSG Coordenacao Comercial com o acesso legítimo do time (incl. flow Alterar Turno); (b) trocar em bloco: atribuir o novo e remover o PSG Administrador dos 5; (c) remover de Isabela (completar PSG Gerencia se faltar algo) e dos 3 externos (acesso elevado de consultoria = permission set com expiração); (d) PSG Administrador restrito aos 4 admins humanos. Inspecionar conteúdo do PSG antes: `SELECT Label, PermissionsManageUsers, PermissionsManageInternalUsers, PermissionsModifyAllData, PermissionsViewAllData FROM PermissionSet WHERE Type = 'Group' AND Label = 'PSG Administrador'` e `SELECT PermissionSet.Label FROM PermissionSetGroupComponent WHERE PermissionSetGroup.MasterLabel = 'PSG Administrador'`
