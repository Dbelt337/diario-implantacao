...
**Modelagem nativa — pesquisa fechada 12/08 (doc completo: hu119-devolucoes-solucao-fundamentada):** o Automotive Cloud não tem objeto de devolução (lista oficial de Standard Objects confirma). As TRÊS opções nativas da plataforma, com o estado real na org:
- `ReturnOrder`/`ReturnOrderLineItem` (objetos dedicados, API 42+): exigem Order Management ou Field Service — **descartado pela mesma decisão que descartou o OMS**;
- **Reduction Orders (RECOMENDADO)**: capacidade do Order padrão (`IsReductionOrder` + `OriginalOrderId`), devolução/anulação **por linha** referenciando as OrderItems originais. Não é objeto — é setting: Setup → Order Settings → "Enable Reduction Orders" (confirmado DESLIGADO na DevSales, print 12/08). Criação via Apex a partir da tela de solicitação própria (a UI padrão do Lightning é incompleta para reduction orders — a nossa é LWC);
- Negative Quantity (JÁ ligado na org): não recomendado como modelo — não gera documento próprio nem vincula linha→linha, perde o espelho 1:1 com o documento SAP.
É o padrão da indústria: o SAP DBM reverte venda criando **returns order referenciando o documento original** ("To reverse the sales transaction, you can create a returns order" — SAP Help Portal, DBM Order Processing); o Z111 da Isabella é uma dessas classes. O Salesforce espelha: solicitação (reduction order) vinculada ao pedido original, motivo obrigatório, aprovação, notificação a Logística; Mule leva ao SAP; retorno pelo canal deployado.
**Pendência antes de escrever na HU:** smoke test de compatibilidade com Enhanced Commerce Orders (ligado e travado na org) — habilitar o setting e criar uma reduction order via Execute Anonymous (script no doc). Se passar, modelo confirmado; se falhar, plano B documentado (objeto de solicitação próprio, exceção justificada por limitação de plataforma).
