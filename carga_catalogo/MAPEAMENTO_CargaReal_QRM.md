...
- DESC-01 (kickoff fase descontos): avaliar o app **Price Management** (Salesforce Pricing) presente na org — Pricing Procedures (waterfall de preço) e **Price Adjustment Schedules** (ajustes com vigência = candidato NATIVO para promos/cashback com Fecha Final, resolvendo a CARGA-01 sem campo custom). Rota preliminar: Decision Matrix p/ alçada de desconto + Adjustment Schedules p/ promos. Decidir no kickoff, não agora.
- CARGA-05 **parcialmente concluída 14/07**: ✅ **VehicleDefinition carregada — 223/223** (1:1 com os produtos via ProductId Unique; Name+año, ModelCode=OCN, EngineCubicCapacity extraída; picklists FuelSource/TransmissionSystem/DrivetrainSystem deixados vazios — valores da org desconhecidos, enriquecer depois via UPDATE se desejado). Q3b PAGA: describe do VehicleDefinition documentado (campos técnicos = Text livre; TransmissionType/FuelType/DrivetrainType DEPRECATED — usar os novos *System/*Source). ⏳ **Vehicle segue vazio e correto**: unidades físicas com VIN só entram com extrato de estoque real do SAP/DMS. Produtos NÃO se carregam no Vehicle — catálogo ≠ estoque.
