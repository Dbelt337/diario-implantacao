...
### W-B2C-02 — [SALESFORCE B2C / VIABILIDADE & MÓVEL] - Análise de Viabilidade Técnica no Lead e Oferta Móvel Obrigatória
1. NARRATIVA DE NEGÓCIO
...
3. ESPECIFICAÇÃO TÉCNICA (SALESFORCE)
- [ADERÊNCIA AO ORG BTP · 19-20/08] Padrão recomendado para a consulta: TMF679 (Product Offering Qualification), certificado no Communications Cloud. O resultado da viabilidade deve criar/atualizar vlocity_cmt__Premises__c + vlocity_cmt__ServicePoint__c — verificados no org: existem, estão zerados (adoção limpa) e o hook PremisesInterface está ativo no motor. O token de reserva de porta mora no ServicePoint.
...
### W-B2C-03 — [SALESFORCE B2C / CRÉDITO & MESA DE CRÉDITO] - Análise de Risco
1. NARRATIVA DE NEGÓCIO
...
3. ESPECIFICAÇÃO TÉCNICA (SALESFORCE)
- [ADERÊNCIA AO ORG BTP · 19-20/08] A fila da Mesa de Crédito já existe no org (CreditTable). Alternativa ao Custom Metadata: Decision Matrix/Expression Sets — o BRE já está habilitado no org. [DEFINIR: comportamento do score exatamente no corte — pendência herdada da Política de Crédito.]
...
### W-B2C-04 — [SALESFORCE B2C / ENDEREÇO & RISCO] - Sinalização de Débito no Endereço (Flag/Relatório) e Histórico sem Trava Impeditiva
- [REVISÃO 20/08] Consolidada com a história "Sinalização de Endereço Inadimplente (Flag) e Relatório Estratégico pós-Venda" (reunião de 18/08) — era a mesma entrega escrita duas vezes. Conteúdo único abaixo; itens exclusivos da segunda versão incorporados.
- Contexto adicional (18/08): prática de "rodízio de CPF" em endereços com dívidas ativas; a proposta de travar a venda com aprovação do coordenador foi descartada por inviabilidade operacional.
1. NARRATIVA DE NEGÓCIO
...
3. ESPECIFICAÇÃO TÉCNICA (SALESFORCE)
- [ADERÊNCIA AO ORG BTP · 19-20/08] Chave de matching do endereço: CEP + número + código IBGE — nunca o complemento. Modelo de endereço verificado no org: Endereco__c (custom) + Address (Field Service) já relacionados. O código IBGE vem de graça na mesma consulta de CEP (ViaCEP).
...
### W-B2C-05 — [SALESFORCE B2C / ANALYTICS & DASHBOARD] - Painel de Controle de Vendas Travadas e Gestão de Perdas Segregada
1. NARRATIVA DE NEGÓCIO
...
3. ESPECIFICAÇÃO TÉCNICA (SALESFORCE)
- [ADERÊNCIA AO ORG BTP · 19-20/08] 100% nativo. Stage_Age_Days__c exige campo + flow de carimbo de data (ou reporting snapshot) — não existe pronto.
...
### W-B2C-06 — [SALESFORCE B2C / CPQ & SEGMENTAÇÃO] - Tipo de Negociação no Início da Oportunidade e Segmentação B2C/B2S por Ticket Médio
1. NARRATIVA DE NEGÓCIO
...
3. ESPECIFICAÇÃO TÉCNICA (SALESFORCE)
- [ADERÊNCIA AO ORG BTP · 19-20/08] O carrinho do org é o CPQ do pacote Vlocity (LWCs cfBTecPar em produção): a segmentação por ticket entra como price rule/pricing plan do pacote executada no reprice — não como Price Rule do Salesforce CPQ padrão.
...
### W-B2C-07 — [SALESFORCE B2C / DELIVERY & FIELD SERVICE] - Regra de Reagendamento e Cancelamento de Instalação por Insucesso de Contato (72h / 3 Tentativas)
1. NARRATIVA DE NEGÓCIO
...
3. ESPECIFICAÇÃO TÉCNICA (SALESFORCE)
- [ADERÊNCIA AO ORG BTP · 19-20/08] Field Service confirmado no org (Address/Location/ServiceAppointment em uso no B2B). O cancelamento em cascata (WorkOrder + Order + Opportunity) é orquestração de flow.
...
- [ ] Testes unitários com cobertura mínima de 85% e sem violação de Governor Limits.
- [ ] Validação dos fluxos/regras em ambiente de Sandbox/QA.
- Massa de Teste Sugerida: Ordem em Delivery com 2 tentativas de contato, Ordem em Delivery atingindo a 3ª tentativa (deve mover para Backoffice), Ordem no Backoffice há 71h, Ordem no Backoffice atingindo 72h (deve ser cancelada automaticamente).
### W-B2C-08 — [SALESFORCE B2C / GOVERNANÇA E COMPLIANCE] - Edição Restrita de Unidade Operacional por Escopo Regional
1. NARRATIVA DE NEGÓCIO
...
3. ESPECIFICAÇÃO TÉCNICA (SALESFORCE)
- [ADERÊNCIA AO ORG BTP · 19-20/08] Picklist dependente nativa não filtra por usuário — o de-para User_Regional_Mapping__c previsto na própria história é o caminho correto (choices filtradas em screen flow/LWC). Provisionamento Senior/Octa: confirmar mecanismo (SSO/SCIM) com a TI.
...
### W-B2C-09 — [SALESFORCE B2C / JORNADA DE CONTRATAÇÃO] - Resumo do Pedido e Seleção da Modalidade de Assinatura
1. NARRATIVA DE NEGÓCIO
- Como Consultor de Vendas B2C
...
3. ESPECIFICAÇÃO TÉCNICA (SALESFORCE)
- [ADERÊNCIA AO ORG BTP · 19-20/08] A trava do botão de perda pós-assinatura usa a mesma mecânica de visibilidade homologada na W0382 (Revisão Diretoria). Biometria facial não é nativa do Salesforce: parceiro externo via API (unico, idwall, Serpro Datavalid), com validade jurídica reconhecida no Brasil.
...
### W-B2C-10 — [SALESFORCE B2C / MARKETING CLOUD] - Régua Automática de Lembretes de Assinatura e Expiração da Oportunidade (SLA 5 Dias)
1. NARRATIVA DE NEGÓCIO
...
- Encerramento Sistemático: Se ao término do 5º dia [DEFINIR: confirmar] o contrato não for assinado, o sistema atualizará o estágio para Perdido por Falta de Assinatura e moverá o registro para o relatório gerencial.
3. ESPECIFICAÇÃO TÉCNICA (SALESFORCE)
- [ADERÊNCIA AO ORG BTP · 19-20/08] [CONFIRMAR: licença Marketing Cloud Engagement + canal WhatsApp ativos.] O disparo em 15 minutos exige entrada na jornada por API Event — a sincronização padrão do MC Connect roda em ciclos de ~15 min e não garante o SLA. Templates WhatsApp dependem de aprovação da Meta (até 24h). O cancelamento no 5º dia é scheduled flow no core e independe do Marketing Cloud.
...
- Então o sistema deve disparar o 1º lembrete automático via WhatsApp/E-mail contendo o link de aceite.
...
### W-B2C-11 — [SALESFORCE B2C / COBRANÇA E CRÉDITO] - Abertura de Ticket de Débito Interno via Zendesk e SLA de Regularização
1. NARRATIVA DE NEGÓCIO
- Como Consultor de Vendas B2C
...
3. ESPECIFICAÇÃO TÉCNICA (SALESFORCE)
- [ADERÊNCIA AO ORG BTP · 19-20/08] Zendesk via Named Credential + REST outbound; baixa bancária inbound do BSS/SAP via Platform Event ou Apex REST. Sem dependência de licença nova.
...
- Scheduled Flow / Batch Apex: Executa varredura diária das oportunidades em Aguardando Pagamento de Débito Interno há 5 [DEFINIR: confirmar] dias para alteração para Closed Lost.
...
### W-B2C-12 — [SALESFORCE B2C / MESA DE CRÉDITO & FIELD SERVICE] - Solicitacão de Desconto na Taxa de Ativação e Agendamento pós-Baixa via Chatbot
1. NARRATIVA DE NEGÓCIO
- Como Consultor de Vendas B2C
...
3. ESPECIFICAÇÃO TÉCNICA (SALESFORCE)
- [ADERÊNCIA AO ORG BTP · 19-20/08] O chatbot consulta e grava slots pelo Field Service: classe Apex AppointmentBookingService.GetSlots exposta via Integration Procedure/Apex REST (padrão documentado). O gate do agendamento pela flag de taxa paga é validação/flow no ServiceAppointment.
...
### W-B2C-13 — [SALESFORCE B2C / CATALOG & CPQ] - Seleção Guiada de Produtos, Formação de Combos/Promoções e Filtro Dinâmico por Endereço
1. NARRATIVA DE NEGÓCIO
...
3. ESPECIFICAÇÃO TÉCNICA (SALESFORCE)
- [ADERÊNCIA AO ORG BTP · 19-20/08] Fatos verificados no org: vlocity_cmt__Promotion__c disponível; o filtro de prateleira está DESLIGADO hoje — ProductAvailability/ProductEligibility rodam nas implementações Default, e as alternativas (FilterAvailability, FilterEligibility, CtxRulesProductsOpen) existem no registro e estão inativas; context rules ativas apenas para preço. Esta work inclui LIGAR e implementar esse filtro, não apenas configurá-lo. Mecanismo (interfaces novas de Availability & Eligibility do Summer '26 vs context rules clássicas) depende da versão do pacote [DEFINIR: versão instalada]. Pré-requisito: código IBGE no endereço/Premises (ver W-B2C-02 e W-B2C-04).
...
- Riscos/Premissas: Performance nas chamadas da API de cálculo do CPQ durante a navegação no carrinho com múltiplos SVAs adicionados; garantia de resposta da API de viabilidade em [DEFINIR: X] segundos para não engargalar o fluxo do vendedor .
6. DEFINITION OF DONE (DoD) & MASSA DE TESTES
...
### W-B2C-14 — [SALESFORCE B2C / JURÍDICO & DOCUMENTOS] - Acervo de Contratos Assinados: Índice no CRM, Repositório Imutável e API Única de Consulta
1. NARRATIVA DE NEGÓCIO
- Como Atendente, Analista Jurídico ou agente de Cobrança
- Quero consultar qualquer contrato assinado do cliente (gerado no legado ou no Salesforce) diretamente na tela da Conta, com a cadeia de versões (termos aditivos) preservada
- Para que o atendimento e as auditorias não dependam do monolito legado e o histórico jurídico sobreviva à migração da plataforma.
2. CONTEXTO E REGRAS DE NEGÓCIO (TELECOM)
- Contexto: Risco crítico apresentado em 20/08 sobre os documentos jurídicos da plataforma legada (TAS/TCS/TSE/TSA): diff de proposta sem entidade, termo aditivo não gerado em mudança de plano e cadeia de versões (FKs PREVIOUS/BECOME) presa ao banco do monolito. O Salesforce não é repositório de documentos: o desenho separa binário, índice e consulta.
- Regras:
- Binário fora do Salesforce: o PDF assinado mora em repositório com imutabilidade (WORM/Object Lock) — S3, SharePoint ou SAP DMS [DEFINIR: repositório].
- Índice no Salesforce: objeto DocumentoJuridico__c com número, tipo (TAS/TCS/TSE/TSA), data de assinatura, cliente/contrato, hash SHA-256 do arquivo, URL do binário e a cadeia de versões como auto-relacionamento (DocumentoAnterior__c).
- API única de consulta: todos os consumidores (Salesforce, legado, Zendesk, auditoria) buscam o binário pela mesma API, que autentica, registra o log de acesso (LGPD) e devolve link pré-assinado com expiração.
- Migração copia, nunca move: backfill do acervo para o repositório sem apagar a origem até o descomissionamento; job incremental sincroniza documentos novos gerados pelo legado durante a coexistência.
- Go-forward: contratos novos nascem pela jornada de contratação (DocGen + assinatura) já indexados.
3. ESPECIFICAÇÃO TÉCNICA (SALESFORCE)
- Objetos Impactados: DocumentoJuridico__c (novo), Account, Contract, Opportunity.
- Automação / Lógica: * FlexCard/LWC na Conta e no Contrato listando a cadeia de documentos com ação "Abrir" (chama a API e abre o link expirável).
- ETL de backfill: extração do monolito (incluindo FKs PREVIOUS/BECOME) -> carga do índice + cópia dos binários com verificação de hash.
- Job delta diário durante a coexistência.
- Integração / APIs: API de Documentos no MuleSoft (GET /documentos/{id}) com URL pré-assinada e log de acesso; escrita de novos documentos via mesma API.
- Segurança e Acessos: leitura do índice por perfis de atendimento; download sempre via API (nunca link direto ao repositório); log de acesso retido para auditoria.
4. CRITÉRIOS DE ACEITE (FORMATO GHERKIN)
- Cenário 1: Consulta de contrato legado pela tela da Conta
- Dado que um cliente possui contrato assinado no sistema legado já migrado para o acervo,
- Quando o atendente abre a Conta e clica no documento no painel de contratos,
- Então o sistema deve abrir o PDF por link expirável, registrar quem acessou e exibir a cadeia de versões (documento anterior/posterior) quando existir.
- Cenário 2: Coexistência sem impacto no legado
- Dado que o monolito ainda gera contratos durante a transição,
- Quando um novo contrato é assinado no legado,
- Então o job de sincronização deve copiá-lo ao repositório e indexá-lo em até [DEFINIR: X horas], sem qualquer alteração no fluxo do sistema legado.
- Cenário 3: Integridade jurídica
- Dado que um documento do acervo é baixado para uma auditoria,
- Quando o hash do arquivo baixado é comparado ao hash gravado no índice,
- Então os valores devem coincidir, comprovando que o documento é o mesmo que foi assinado.
5. DEPENDÊNCIAS E RISCOS
- Dependências: definição do repositório-alvo [DEFINIR]; volumetria do acervo (quantidade de documentos e GB) [DEFINIR]; acesso de extração ao banco do monolito e ao SAP para os documentos e as FKs de versão.
- Riscos/Premissas: qualidade das FKs PREVIOUS/BECOME no legado pode exigir reconciliação manual de parte da cadeia; documentos órfãos (sem cliente identificável) precisam de fila de tratamento.
6. DEFINITION OF DONE (DoD) & MASSA DE TESTES
- [ ] Objeto de índice criado com cadeia de versões e hash.
- [ ] API de consulta homologada com log de acesso e link expirável.
- [ ] Backfill executado com conferência de hash e contagem origem x destino.
- [ ] Painel na Conta/Contrato homologado pelo Jurídico.
- Massa de Teste Sugerida: contrato simples sem versões; contrato com termo aditivo (cadeia de 2+ documentos); documento do legado ainda não sincronizado (deve aparecer após o job); comparação de hash pós-download.