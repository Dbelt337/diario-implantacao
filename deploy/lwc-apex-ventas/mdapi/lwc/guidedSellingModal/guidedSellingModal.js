...
import QuoteConfirmModal from 'c/quoteConfirmModal';
import DiscountApprovalModal from 'c/discountApprovalModal';
...
export default class GuidedSellingModal extends LightningModal {
    @api recordId;
    ...
    async handleEnviarAprobacion() {
        const result = await DiscountApprovalModal.open({
            size: 'small',
    ...
    async handleCreateQuote() {
        const result = await QuoteConfirmModal.open({
            size: 'small',
