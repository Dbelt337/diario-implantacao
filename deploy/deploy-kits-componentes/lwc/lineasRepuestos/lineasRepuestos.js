...
export default class LineasRepuestos extends LightningElement {
    ...
    @track lineas = [];
    /** Vista plana del grid: cada línea y, debajo, sus componentes de kit (GQ-PV-02-029). */
    @track filas = [];
    codigoNuevo = '';
    cantidadNueva = 1;
    cargaMasiva = '';
    ...
    refrescarVista() {
        this.lineas = this.lineas.map((l) => ({ ...l, ...this.decorar(l, l.ventaPerdida === true) }));
        // vista plana: la línea del asesor y, debajo, los componentes del kit
        // (solo lectura — la cantidad del componente deriva de la del kit)
        const filas = [];
        this.lineas.forEach((l) => {
            filas.push({ ...l, clave: l.productCode, editable: true, esPadre: l.esKit === true });
            (l.componentes || []).forEach((c) => {
                filas.push({
                    ...c, ...this.decorar(c, false),
                    clave: `${l.productCode}>${c.productCode}`,
                    editable: false, esPadre: false, mostrarCrearMaterial: false,
                    rowClass: 'fila-componente'
                });
            });
        });
        this.filas = filas;
    ...
    decorar(l, perdida) {
        return {
            badgeClass: perdida ? 'slds-badge slds-theme_error' : (BADGE[l.status] || 'slds-badge slds-badge_lightest'),
            statusLabel: perdida ? 'Venta Perdida' : (l.status || 'Sin consultar'),
            saldoLabel: l.saldoDisponible === undefined || l.saldoDisponible === null
                ? '—' : `${l.saldoDisponible} (piso ${l.piso} / reserva ${l.reserva})`,
            perdidaVariant: perdida ? 'brand' : 'neutral',
