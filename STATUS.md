...
- **OpportunityBeforeHandler "não entrou" (06/08)**: o deploy log de ontem diz created:true (id 301WK00002VOimjYAD) — provável confusão de busca: a lista de Flows mostra o LABEL "Flow Trigger: Opportunity Before Handler", não o API name. Reenviado zip standalone `HU046_flow_opphandler.zip` (idempotente) para garantir.
