...
- **Decisão de ARQUITETURA (pergunta do Diego: "está certo consultar nesse objeto?")**: NÃO. SerializedProduct era resquício das premissas antigas da HU (pré-pivot para Vehicle.Status). No modelo Automotive a unidade é o par **Vehicle+Asset** e a identidade é `Vehicle.VehicleIdentificationNumber`; SerializedProduct é objeto opcional de inventário serializado (Manufacturing), NÃO está no mapa de cargas SAP — exigi-lo obrigaria toda carga a criar um 3º objeto por unidade com SerialNumber=VIN (duplicação de identidade, viola regra 1 do modelo). Evidência extra: o apex de teste falhou com "Field does not exist: ProductId on SerializedProduct" nesta org.
