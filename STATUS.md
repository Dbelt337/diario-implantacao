...
- **Solicitação com inventário real**: mesma tela; wire B v2 deriva marca (VehicleDefinition→Product2→BusinessBrand) e sucursal (CurrentOwnerId→Account) da unidade, cupo conta por marca+sucursal, e adicionar validação de elegibilidade (só "En ubicación de concesionario" pode ser solicitada — hoje v1 aceita re-solicitar unidade já em demo).
