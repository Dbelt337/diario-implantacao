...
**Descoberta no 1º teste**: busca por "RPA Toyota Hilux" (nome) → "Unidad no encontrada". Query confirmou Names intactos → a hipótese é que a versão ATIVA busca pelo campo `VehicleIdentificationNumber` (ou o inverso — em verificação testando com `VIN10001`). **A versão com busca DUPLA (Name OU VIN, filterLogic OR) já está pronta nos zips `HU046_flow_manage.zip` + `HU046_flow_execdemo.zip` (commit 5cfd10c) — deploy + ativação ficam para DEPOIS do ciclo de teste atual** (não trocar de versão no meio do teste).
